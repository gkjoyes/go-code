...
func main() {
	...
	// Store the traceID value inside the context with a value of zero
	// of type TraceIDKey for the key type.
	...
	// Retrieve that traceID value from the Context value bag not using the
	// proper key type.
	if _, ok := ctx.Value(traceIDKey).(TraceID); ok {
