// Sample program that takes a stream of bytes and looks for the bytes "elvis"
// and when they are found, replace them with "Elvis". The code cannot assume that
// there are any line feeds or other delimiters in the stream and code must assume
// that the stream is of any arbitrary length. The solution cannot meaningfully
// buffer to the end of the stream and then process the replacement.
...
// data respresent a table of inputs and expected output.
var data = []struct {
	input  []byte
	output []byte
}{
...
// assembleOutputStream combines all the output into a single stream for comparing.
...
// algOne is one way to solve problem.
func algOne(data, find, repl []byte, output *bytes.Buffer) {
	...
	for {
		...
		if _, err := io.ReadFull(input, buf[end:]); err != nil {
			...
			// Flush the rest of the bytes we have.
...
func algTwo(data, find, repl []byte, output *bytes.Buffer) {
	...
	// Creat an index variable to match bytes.
	idx := 0
	...
	for {
		...
		if b == find[idx] {
			...
			// If every byte has been matched, write out the replacement.
			if idx == size {
				output.Write(repl)
				idx = 0
			}
		...
		// Did we have any sort of match on any give byte?
		if idx != 0 {
