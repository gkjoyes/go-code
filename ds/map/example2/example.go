...
func main() {
	...
	// Read the element at key "anna". It is absent so we
	// get the zero-value for this map's value type.
	score := scores["anna"]
	fmt.Println(score)

	// If we need to check for the presence of a key we can use
	// a 2 variable assignment. The second variable is a bool.
	...
	// Without this behavior we would have to code in
